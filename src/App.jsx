  ...
  const startTimer = () => {
    if (!isRunning) {
      ...
      timerRef.current = setInterval(() => {
        ...
        setTime(elapsed);
        localStorage.setItem("time", JSON.stringify(elapsed)); // Save milliseconds
      }, 10);
    }
  };
    ...
    >
      <div className="flex text-[20px] gap-1 font-bold ">
        ...
        <button
          ...
          className={`w-8 h-8 bg-[#212121] p-1 transition-transform transform duration-300  rounded-full mb-4 ${
        ...
        >
          ...
          <svg
            ...
            viewBox="0 0 24 24"
            fill="currentColor"
            className={` ${dark ? "rotate-180 block" : "rotate-0 hidden"}`}
          >
      ...
      >
        <span className="sm:w-[180px] text-center">
          {String(hr).padStart(2, "0")}
        </span>
        <span>:</span>
        <span className="sm:w-[180px] min-w-[90px] text-center">
